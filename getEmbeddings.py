...
def removeNum(text):
    text= text.dropna(how="all")  #행 내의 값이 모두 null인 경우 해당 행을 삭제
    text=text.drop(text.isnull())
    return text
...
def dat_embedding(path,vector_dimension=300):
    """
    ...
    x = constructLabeledSentences(data['text']) #신문 기사 내용
...
def clean_data():
    ...
    del_rows=[]     #to-do 삭제할 데이터, label 이 1과 0 아니면
    for i in range(len(data)):
        if data.loc[i, 'label'] != 0 and \
          data.loc[i, 'label'] != 1:
            del_rows.append(i)
        data = data.drop (del_rows, 0)
